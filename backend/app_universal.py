...
# SECURITY FIX: Users are now loaded from environment or database
# DO NOT hardcode passwords in source code
def get_users_from_env():
    """Load users from environment variables for security"""
    users = {}
    # Format: USER_1_EMAIL, USER_1_PASSWORD, USER_1_TENANT, USER_1_NAME, USER_1_DATA_DIR
    import os
    for i in range(1, 10):  # Support up to 9 users
        ...
        if email:
            users[email] = {
                ...
                "tenant": os.getenv(f"USER_{i}_TENANT", "default"),
                "name": os.getenv(f"USER_{i}_NAME", email),
                "data_dir": os.getenv(f"USER_{i}_DATA_DIR", "data")
            }
    return users
...
# SECURITY FIX: All credentials loaded from environment variables
import os
...
if not AZURE_OPENAI_ENDPOINT:
    print("[WARNING] AZURE_OPENAI_ENDPOINT not set - some features may not work")
if not AZURE_OPENAI_API_KEY:
    print("[WARNING] AZURE_OPENAI_API_KEY not set - some features may not work")
