"""
Google Slides Connector for 2nd Brain
Syncs Google Slides presentations via Google Drive API
"""
...
class GSlidesConnector(BaseConnector):
    """Connector for Google Slides - syncs only Google Slides presentations"""

    CONNECTOR_TYPE = "gslides"
        ...
        try:
            ...
            if self.credentials.expired and self.credentials.refresh_token:
                try:
                    ...
                    print(f"[GSlides] Token refresh failed: {e}")
    ...
    def sync(self, since: Optional[datetime] = None) -> List[Document]:
        ...
        try:
            ...
            query_parts = [f"mimeType='{GSLIDES_MIME}'", "trashed=false"]
            if since:
                    ...
                    if doc:
                        ...
                        if self.on_document_ready:
                            try:
                                ...
                                print(f"[GSlides] on_document_ready error: {cb_err}")
                        file_count += 1
            ...
            self.status = ConnectorStatus.CONNECTED
            print(f"[GSlides] Sync complete: {len(documents)} presentations")
        except Exception as e:
    ...
    def get_document(self, doc_id: str) -> Optional[Document]:
        if not self.service:
            self.connect()
        try:
            file_id = doc_id.replace("gslides_", "")
    ...
    def _file_to_document(self, file: Dict[str, Any]) -> Optional[Document]:
        try:
            ...
            return Document(
                doc_id=f"gslides_{file_id}",
                source="gslides",
                content=content,
                title=name,
                metadata={"file_id": file_id, "mime_type": GSLIDES_MIME, "owners": owner_names, "user": self.user_email},
                timestamp=timestamp,
            ...
            print(f"[GSlides] Error converting file: {e}")
            return None
    ...
    def _extract_content(self, file_id: str) -> Optional[str]:
        try:
            ...
            print(f"[GSlides] Export failed for {file_id}: {e}")
            return None
