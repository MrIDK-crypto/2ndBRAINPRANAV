...
class SlackConnector(BaseConnector):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if response["ok"]:
                ...
                if url:
                    # Parse domain from URL like "https://myteam.slack.com/"
                    import re
                    match = re.search(r'https://([^.]+)\.slack\.com', url)
                    if match:
                        self.team_domain = match.group(1)
                        print(f"[Slack] Team domain: {self.team_domain}", flush=True)
    ...
    async def _message_to_document(
        ...
        try:
            ...
            # Check both subtype AND bot_id field (bots post with bot_id, not always subtype)
...
{text}"""
            ...
            return Document(
                ...
                metadata={
                    ...
                    "team_domain": getattr(self, 'team_domain', None),
    ...
    def _sync_sync(self, since: Optional[datetime] = None) -> List[Document]:
        """Synchronous version of sync for use with gevent workers"""
        if not self.client:
            self._connect_sync()
        ...
        try:
            # Get channels to sync
            channels = self._get_channels_sync()
            ...
            for channel in channels:
                channel_docs = self._sync_channel_sync(channel, oldest)
                documents.extend(channel_docs)
    ...
    def _get_channels_sync(self) -> List[Dict]:
        """Synchronous get channels"""
        channels = []
        configured_channels = self.config.settings.get("channels", [])
    ...
    def _sync_channel_sync(self, channel: Dict, oldest: Optional[float]) -> List[Document]:
        """Synchronous sync channel"""
        documents = []
        max_messages = self.config.settings.get("max_messages_per_channel")
        ...
        try:
            ...
            while True:
                ...
                for message in messages:
                    doc = self._message_to_document_sync(message, channel)
                    if doc:
                        documents.append(doc)

                    if (self.config.settings.get("include_threads", True) and
                        message.get("reply_count", 0) > 0):
                        thread_docs = self._sync_thread_sync(channel, message["ts"])
                        documents.extend(thread_docs)
    ...
    def _sync_thread_sync(self, channel: Dict, thread_ts: str) -> List[Document]:
        """Synchronous sync thread"""
        documents = []
        ...
        try:
            ...
            for message in response.get("messages", [])[1:]:
                doc = self._message_to_document_sync(message, channel, is_reply=True)
                if doc:
                    documents.append(doc)
    ...
    def _message_to_document_sync(
        self,
        message: Dict,
        channel: Dict,
        is_reply: bool = False
    ) -> Optional[Document]:
        """Synchronous message to document conversion"""
        try:
            # Skip bot messages and system messages
            # Check both subtype AND bot_id field (bots post with bot_id, not always subtype)
            if message.get("subtype") in ["bot_message", "channel_join", "channel_leave"]:
            ...
            user_id = message.get("user", "Unknown")
            author = self._get_user_name_sync(user_id)

            ts = float(message.get("ts", 0))
            timestamp = datetime.fromtimestamp(ts) if ts else None

            text = message.get("text", "")
            text = self._replace_user_mentions_sync(text)
...
{text}"""
    ...
    def _replace_user_mentions_sync(self, text: str) -> str:
        """Synchronous replace user mentions"""
        import re

        pattern = r'<@([A-Z0-9]+)>'
        matches = re.findall(pattern, text)
        for user_id in matches:
            name = self._get_user_name_sync(user_id)
            text = text.replace(f"<@{user_id}>", f"@{name}")
