    ...
    def _calculate_reproducibility_score(self, steps: List, issues: List, evidence: Dict) -> Dict:
        ...
        for m in mismatches:
            if m.get("severity") == "high":
                breakdown["context_mismatch"] += 20
            elif m.get("severity") == "medium":
                breakdown["context_mismatch"] += 10

        # Issue penalties
        for issue in issues:
            if issue.get("risk_level") == "high":
                breakdown["high_risk_issues"] += 15
            elif issue.get("risk_level") == "medium":
                breakdown["medium_risk_issues"] += 8
            else:
                breakdown["low_risk_issues"] += 3

        # Corpus support
        corpus = evidence.get("corpus_results", {})
        if corpus.get("stats", {}).get("cooccurrence_count", 0) < 5:
            breakdown["no_corpus_support"] = 10

        # Calculate current score
        total_deduction = sum(breakdown.values())
        current_score = max(0, base_score - total_deduction)

        # Potential score after optimization (assume 80% of issues fixed)
        potential_recovery = (breakdown["high_risk_issues"] + breakdown["medium_risk_issues"]) * 0.8
        after_score = min(100, current_score + potential_recovery)
